...
// ------------ load consultants ------------
$consultants = [];
$res2 = @mysqli_query($conn, "SELECT id, name FROM consultants ORDER BY name");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($consultant_id !== '') {
        ...
        foreach ($consultants as $c) {
            if ((string)$c['id'] === (string)$consultant_id) {
                $valid = true;
                $consultant_doctor = $c['name']; // keep consistent
                break;
            }
        }
        if (!$valid) {
    ...
    if (empty($errors)) {
        // ***** IMPORTANT PART: 20 placeholders, 20 params *****
        $sql = "UPDATE patients SET
                    ...
                    consultant_id          = NULLIF(?, ''),
                    consultant_doctor      = NULLIF(?, '')
                WHERE id = ?";

        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            // 19 's' params + 1 'i' (id)  = 20 total
            $types = str_repeat('s', 19) . 'i';
