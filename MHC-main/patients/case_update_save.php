...
if ($case_update_id > 0) {
    ...
    $sql = "UPDATE case_update SET
        patient_id = ?, case_id = ?, consultant_id = ?, follow_up_no = ?, record_date = ?,
    ...
    // types: iiii (patient_id, case_id, consultant_id, follow_up_no) + 18 strings + i (case_update_id at end)
    $types = 'iiii' . str_repeat('s', 18) . 'i';
    ...
    // types: iiii (first 4 ints) + 18 strings
    $types = 'iiii' . str_repeat('s', 18);
